...
class VehiculeItem(QGraphicsPolygonItem):
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent | None) -> None:
        if event.button() == Qt.RightButton:
            ...
            # details_dialog = MaFenetreSecondaire(self)
            # details_dialog.setParent(self.scene.views()[0].window())  # Utilisez la vue associée à la scène comme parent
            # details_dialog.exec_()

            # details_dialog = MaFenetreSecondaire(self)
            # details_dialog.setParent(self.scene.views()[0])  # Utilisez la vue associée à la scène comme parent
            # details_dialog.exec_()
